...
A desktop-first poker analytics application that empowers serious poker players to analyze their hand histories, understand player tendencies, calculate probabilities in real-time, and track long-term performance. The platform combines a rigorous poker engine with modern analytics and an AI-powered assistant to surface insights that players would otherwise miss.

### Elevator Pitch

"Poker Analytics Platform is the bridge between raw hand histories and winning poker decisions. Store every hand. Analyze every player. Find every leak. Win more."
...
| Area | Description |
|------|-------------|
| Hand History Import | Parse and store Texas Hold'em hand histories from major online platforms. MVP: PokerStars, GGPoker, Ignition/Bovada. Architecture designed to add Winamax, PartyPoker, iPoker, and others with zero schema/API/frontend changes — only a new parser class. |
| Hand History Storage | Full relational storage of every hand, action, street, card, and result |
| Poker Engine | Hand ranking, equity calculation, outs calculation, Monte Carlo simulation |
...
| Hand Explorer | Browse, filter, search, and replay hand histories |
| AI Assistant | Natural language queries over stored data, statistic explanations, basic leak identification |

### 2.2 Out of Scope (MVP)

- Omaha support (architecture designed to accommodate it)
...
| Phase | Features |
|-------|----------|
| Phase 2 | Omaha support, tournament tracking, ICM calculations |
...
- Plays 30–50 hours/week across multiple sites
...
**Taylor — The Coach**
- Reviews hands for multiple students
...
├── poker-engine/                   # Pure Python poker library (zero deps outside stdlib)
│   ├── src/
...
1. **Poker engine is pure.** It knows nothing about databases, HTTP, or UIs. It takes inputs and returns outputs. 100% testable without infrastructure.
...
| Poker Domain Expert | Validate poker math, hand ranking correctness, stat definitions |
